...
- `#token` - Workspace token obtained when the application is installed into Slack workspace (create a new [Workspace app](https://api.slack.com/workspace-apps-preview), go to `OAuth & Permissions` then click `Install App` to obtain the token. Don't forget to set Scopes & Permissions below to `Post messages as the app`.
